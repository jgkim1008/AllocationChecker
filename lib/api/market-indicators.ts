export type AlertLevel = 'danger' | 'warning' | 'neutral' | 'positive';
...
async function fetchSymbol(cfg: IndicatorConfig): Promise<MarketIndicator> {
...
/**
 * 11개 매크로 지표(VIX·금리·환율·원자재·SOX·러셀2000) 조회
 * market-indicators API 라우트와 briefing 대시보드가 공용으로 사용
 */
export async function getMarketIndicators(): Promise<MarketIndicatorsResult> {
  ...
  return { indicators, marketComment, updatedAt: new Date().toISOString() };
}
