...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[balance] params:', { credentialId: credentialId?.slice(0, 8), brokerType, includeOverseas, effectiveBrokerType, isKIS: client instanceof KISClient });
    if ((effectiveBrokerType === 'kis' || brokerType === 'kis') && includeOverseas && client instanceof KISClient) {
      ...
      console.log('[balance] overseas BEFORE present-balance override:', {
        totalAsset: fullData.overseas.balance.totalAsset,
        totalEval: fullData.overseas.balance.totalEvalAmount,
        totalBuy: fullData.overseas.balance.totalBuyAmount,
        totalPnL: fullData.overseas.balance.totalProfitLoss,
      });

      // 해외 잔고에 USD 예수금 합산 (정상 조회 시)
      if (presentResult.success && presentResult.data) {
        const foreignCash = presentResult.data.foreignCash || 0;
        console.log('[balance] overseas foreign cash (USD):', foreignCash);
      ...
      console.log('[balance] overseas AFTER override:', {
        totalAsset: fullData.overseas.balance.totalAsset,
        totalDeposit: fullData.overseas.balance.totalDeposit,
      });
