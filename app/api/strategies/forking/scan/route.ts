...
export interface ForkingStock {
  ...
  forkSpread: number;        // (ma5 - ma10) / ma10 * 100
  forkingSpeed: number;      // forkSpread - prevForkSpread (확대 = 양수)
...
type SignalCategory = 'FORKING' | 'SELL';

function getSignalCategory(ma5: number, ma10: number): SignalCategory {
  return ma5 > ma10 ? 'FORKING' : 'SELL';
}

function analyzeForking(
  ...
  // 이전 달 신호 카테고리
  const currentCategory = getSignalCategory(ma5, ma10);
  const prevCategory    = getSignalCategory(ma5Prev, ma10Prev);
  const signalChanged   = currentCategory !== prevCategory;
  ...
  for (let i = lastIdx - 1; i >= 20; i--) {
    const ma5AtI  = calcMA(closes, 5,  i);
    const ma10AtI = calcMA(closes, 10, i);
    if (!ma5AtI || !ma10AtI) break;

    const categoryAtI = getSignalCategory(ma5AtI, ma10AtI);
    if (categoryAtI === currentCategory) {
      consecutiveMonths++;
    } else {
...
export async function GET(_req: NextRequest) {
  ...
  try {
    ...
    // 2. 새로 스캔
    console.log(`[Forking] Starting fresh scan for ${TARGET_STOCKS.length} stocks...`);
    const results: ForkingStock[] = [];

    for (const stock of TARGET_STOCKS) {
      const candles = await fetchMonthlyCandles(stock.yahooSymbol);
      if (!candles) {
        console.warn(`[Forking] No candles for ${stock.symbol}`);
        continue;
      }
      const analyzed = analyzeForking(stock, candles);
    ...
    console.error('[Forking Scan API Error]', error);
