...
import {
  RefreshCw, ChevronRight, ChevronUp, ChevronDown, Activity, BarChart2,
} from 'lucide-react';
import { PremiumGate } from '@/components/PremiumGate';
import { getClientCache, setClientCache, clearClientCache } from '@/lib/client-cache';
import type { EWStock } from '@/app/api/strategies/elliott-wave/scan/route';
import type { EWSignal } from '@/lib/utils/elliott-wave-calculator';

const CACHE_KEY = '/api/strategies/elliott-wave/scan';
...
const SIGNAL_META: Record<EWSignal, { label: string; cls: string }> = {
  WAVE2_END:    { label: '파동2 완료',    cls: 'bg-cyan-100 text-cyan-700' },
  WAVE4_END:    { label: '파동4 완료',    cls: 'bg-purple-100 text-purple-700' },
  WAVE3_ACTIVE: { label: '파동3 진행중',  cls: 'bg-emerald-100 text-emerald-700' },
  WAVE5_END:    { label: '파동5 완료',    cls: 'bg-amber-100 text-amber-700' },
  ABC_END:      { label: 'ABC 조정 완료', cls: 'bg-blue-100 text-blue-700' },
  UNCLEAR:      { label: '불명확',        cls: 'bg-gray-100 text-gray-500' },
};
              ...
              : benchmarks.map(b => (
                  <tr
                    ...
                    onClick={() => {
                      ...
                      if (nav) router.push(`/strategies/elliott-wave/${nav.symbol}?market=${nav.market}&name=${encodeURIComponent(nav.name)}`);
                    }}
                    className="border-b border-gray-50 hover:bg-cyan-50 transition-colors cursor-pointer group"
                  >
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: b.color }} />
                        <span className="text-xs font-bold text-gray-700 group-hover:text-cyan-700 transition-colors">{b.name}</span>
                      </div>
                    </td>
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-1 ${align === 'right' ? 'justify-end' : align === 'center' ? 'justify-center' : 'justify-start'}`}>
        <span>{label}</span>
        <span className={isActive ? 'text-cyan-600' : 'text-gray-300'}>
...
// ── 종목 행 ─────────────────────────────────────────────────────────────
function StockRow({ stock }: { stock: EWStock }) {
  const router = useRouter();
  const meta = SIGNAL_META[stock.signal];

  return (
    <tr
      onClick={() => router.push(`/strategies/elliott-wave/${encodeURIComponent(stock.symbol)}?market=${stock.market}&name=${encodeURIComponent(stock.name)}`)}
      className={`border-b transition-colors cursor-pointer group ${
        stock.signal === 'WAVE2_END'
          ? 'bg-cyan-50/60 hover:bg-cyan-100/60'
          : stock.signal === 'WAVE4_END'
            ? 'bg-purple-50/60 hover:bg-purple-100/60'
            : stock.signal === 'WAVE3_ACTIVE'
    ...
    >
      ...
      <td className="px-3 py-3 text-center">
        <span className="text-xs font-black text-gray-700">
          {stock.currentWave !== null ? `파동 ${stock.currentWave}` : '-'}
        </span>
      </td>
      ...
      <td className="px-3 py-3 text-right">
        <span className="text-xs text-emerald-600 font-mono">
          {stock.targetPrice ? formatPrice(stock.targetPrice, stock.market) : '-'}
        </span>
      </td>
      ...
      <td className="px-3 py-3 text-right">
        <span className="text-xs text-red-500 font-mono">
          {stock.stopLoss ? formatPrice(stock.stopLoss, stock.market) : '-'}
        </span>
      </td>
...
export default function ElliottWavePage() {
  const [stocks, setStocks] = useState<EWStock[]>([]);
  ...
  const fetchData = useCallback(async (force = false) => {
    if (!force) {
      const cached = getClientCache<{ stocks: EWStock[]; timestamp: string }>(CACHE_KEY);
    ...
    setError(null);
    try {
      const res = await fetch(`/api/strategies/elliott-wave/scan${force ? '?refresh=1' : ''}`);
  ...
  const SIGNAL_PRIORITY: Record<EWSignal, number> = {
    WAVE2_END: 5, WAVE4_END: 4, WAVE3_ACTIVE: 3, WAVE5_END: 2, ABC_END: 2, UNCLEAR: 1,
  };

  const unique = stocks.filter((s, i, arr) =>
    arr.findIndex(x => x.symbol === s.symbol && x.market === s.market) === i
  );
  const sorted = [...unique].sort((a, b) => {
    let diff = 0;
    switch (sortKey) {
      case 'signal':   diff = SIGNAL_PRIORITY[b.signal] - SIGNAL_PRIORITY[a.signal]; break;
  ...
  const w2Count = unique.filter(s => s.signal === 'WAVE2_END').length;
  const w4Count = unique.filter(s => s.signal === 'WAVE4_END').length;
  const w3Count = unique.filter(s => s.signal === 'WAVE3_ACTIVE').length;
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-black text-gray-900">엘리어트 파동</h1>
              <p className="text-sm text-gray-500 mt-1">ZigZag 피벗 기반 5파 임펄스 패턴 탐지 (파동2/4 완료 → 파동3/5 진입)</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-2xl border border-cyan-100 p-4">
              <p className="text-xs text-gray-500 font-bold">파동2 완료</p>
              <p className="text-2xl font-black text-cyan-600 mt-1">{loading ? '-' : w2Count}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">파동3 진입 (최강 신호)</p>
            </div>
            <div className="bg-white rounded-2xl border border-purple-100 p-4">
              <p className="text-xs text-gray-500 font-bold">파동4 완료</p>
              <p className="text-2xl font-black text-purple-600 mt-1">{loading ? '-' : w4Count}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">파동5 진입</p>
            </div>
            <div className="bg-white rounded-2xl border border-emerald-100 p-4">
              <p className="text-xs text-gray-500 font-bold">파동3 진행중</p>
              <p className="text-2xl font-black text-emerald-600 mt-1">{loading ? '-' : w3Count}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">모멘텀 확인 구간</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100 bg-gray-50">
              ...
              <span className="text-xs text-gray-400 ml-auto">{loading ? '로딩 중...' : `${unique.length}개 종목`}</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="px-3 py-2.5 text-center text-[10px] font-black text-gray-400 uppercase tracking-wider">현재 파동</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-black text-gray-400 uppercase tracking-wider">목표가</th>
                    <th className="px-3 py-2.5 text-right text-[10px] font-black text-gray-400 uppercase tracking-wider">손절가</th>
                    <th className="px-2 py-2.5 w-8" />
                  </tr>
                </thead>
                <tbody>
                        ...
                        <tr key={i} className="border-b border-gray-50">
                          {Array.from({ length: 8 }).map((__, j) => (
                            <td key={j} className="px-3 py-3">
                              <div className="h-4 bg-gray-100 rounded animate-pulse" style={{ width: j === 0 ? 72 : j === 1 ? 80 : 56 }} />
                            </td>
                          ))}
                        </tr>
                      ))
